...
function describeModels(app) {
  ...
  app.handler('rest').adapter.getClasses().forEach(function(c) {
    ...
    c.settings = app.models[name].settings;
    result[name] = c;
  });
...
function buildScopeMethod(models, modelName, method) {
  ...
  if (modelClass.scopes[scopeName] === undefined) {
    ...
    targetClass = findTargetModelName(models, targetClass);
...
function findTargetModelName(models, name) {
  var lowerCaseName = name.toLowerCase();
  ...
  for (var n in models) {
    var relations =  models[n].settings.relations;

    for (var r in relations) {
      if (-1 < r.toLowerCase().indexOf(lowerCaseName)) {
        return relations[r].model;
      }
    }
  }
}
